...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <div>
            ...
            <form action="{{ route('eleves_par_classe_comptable') }}" method="GET" id="filterForm">
                @csrf
                    ...
                    <div class="table-container mt-3">
                        ...
                        <div class="table-wrapper">
                            <div class="input-group">
                                <input type="text" class="form-control" id="searchElevesInput"
                                    placeholder="Rechercher un élève..."><span class="input-group-text bg-primary text-white"><i
                                        class="fas fa-search"></i></span>
                            </div>
                            <table class="table table-striped" id="payTable1">
                                <thead>
                                    <tr>
                                        <th>#ID</th>
                                        <th>Matricule</th>
        ...
        <div class="mt-5">
            ...
            <div class="d-flex gap-2 align-items-end mb-3 flex-wrap">
                ...
                <div style="min-width:220px;">
                    ...
                    <select id="fraisFilter" class="form-select">
                        ...
                        @foreach ($fraisList as $f)
                            <option value="{{ $f->id }}">{{ $f->designation }}
                                [{{ $f->classe->designation ?? '-' }}]</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div id="resultsSummary" class="ms-auto text-end d-flex flex-column align-items-end"
                    style="display:none;">
                        ...
                        <tbody>
                            @forelse($les_paiements as $p)
                                <tr data-statut="{{ $p->statut ?? '' }}" data-frais-id="{{ $p->frais?->id ?? '' }}"
                                    ...
                                    <td>{{ $p->frais?->designation ?? '—' }} <br>
                                        <small class="text-light bg-info px-2 py-1 rounded">
                                            {{ substr($p->frais?->classe->designation ?? '—', 0, 8) }}...
                                        </small>
                                    </td>
                                    <td>
                                        @php
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const searchElevesInput = document.getElementById('searchElevesInput');
            const elevesTable = document.getElementById('payTable1');

            if (searchElevesInput && elevesTable) {
                searchElevesInput.addEventListener('input', function() {
                    const query = this.value.toLowerCase().trim();

                    elevesTable.querySelectorAll('tbody > tr').forEach(row => {
            ...
            function filterPayments() {
                ...
                const hasFilter = q !== '' || status !== '' || classeId !== '' || fraisId !== '';

                // Récupérer toutes les lignes du tbody (exclure les lignes vides ou d'en-tête)
                ...
                rows.forEach(row => {
                    // Ignorer les lignes qui contiennent un message vide ou les lignes d'en-tête
                    if (row.querySelector('.empty-state') || row.innerText.trim() === '') {
                        return;
                    }
                ...
                // Afficher le résumé seulement si un filtre est appliqué
                if (resultsSummary) {
                    resultsSummary.style.display = hasFilter ? 'flex' : 'none';
                }

                // Afficher un message si aucun résultat
                const emptyStateRow = tbody.querySelector('.empty-state-tr');
                if (count === 0 && totalRows > 0) {
                    // Si aucun résultat, afficher un message "Aucun résultat"
                    if (!emptyStateRow) {
                        const newRow = document.createElement('tr');
                        newRow.className = 'empty-state-tr';
                        newRow.innerHTML = `
            ...
            if (classeFilter) {
                classeFilter.addEventListener('change', filterPayments);
            }
            if (fraisFilter) {
                fraisFilter.addEventListener('change', filterPayments);
            }
        ...
        // Fonction améliorée pour confirmer la suppression
        function confirmDelete(e) {
            e.preventDefault();
            if (confirm('⚠️ Êtes-vous sûr de vouloir supprimer ce paiement ? Cette action est irréversible.')) {
                e.target.submit();

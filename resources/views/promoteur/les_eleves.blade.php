...
@section('content')
        ...
        <div class="page-header">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                <div>
                    ...
                    <p>Liste des élèves et gestion des affectations</p>
                </div>
                <div>
                    <button class="btn-modern btn-primary-modern" data-bs-toggle="modal" data-bs-target="#createEleveModal"><i
                            class="fas fa-plus-circle"></i> Nouvel Élève</button>
                </div>
            </div>
        </div>
            ...
            <table class="table table-striped" id="eleveTable">
                ...
                <tbody>
                    @forelse($les_eleves as $eleve)
                            ...
                            <td>
                                <div class="action-buttons">
                                    ...
                                    <button class="btn-icon btn-edit" data-bs-toggle="modal"
                                        data-bs-target="#editEleveModal{{ $eleve->id }}"><i
                                            class="fas fa-edit"></i></button>
                                    <form action="{{ route('eleves.destroy', $eleve->id) }}" method="POST"
                                        onsubmit="return confirmDelete(event)">@csrf @method('DELETE')
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                            data-bs-target="#editEleveModal{{ $eleve->id }}" data-bs-dismiss="modal">
                        ...
                        <!-- Edit modal -->
                        <div class="modal fade modal-modern" id="editEleveModal{{ $eleve->id }}" tabindex="-1">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title">Modifier Élève</h5><button class="btn-close"
                                    ...
                                    <form action="{{ route('eleves.update', $eleve->id) }}" method="POST"
                                        ...
                                        <div class="modal-body">
                                            <div class="form-group"><label class="form-label">Nom</label><input
                                                    name="nom" class="form-control-modern"
                                                    value="{{ $eleve->nom }}" required></div>
                                            <div class="form-group"><label class="form-label">Matricule</label><input
                                                    name="matricule" class="form-control-modern"
                                                    value="{{ $eleve->matricule }}" readonly></div>
                                            <div class="form-group"><label class="form-label">Date de
                                                    naissance</label><input type="date" name="date_n"
                                                    class="form-control-modern"
                                                    value="{{ $eleve->date_n?->format('Y-m-d') }}"></div>
                                            <div class="form-group"><label class="form-label">Lieu de
                                                    naissance</label><input name="lieu_n" class="form-control-modern"
                                                    value="{{ $eleve->lieu_n }}"></div>
                                            <div class="form-group"><label class="form-label">Sexe</label>
                                                <select name="sexe" class="form-control-modern">
                                                    <option value="">--Choisir--</option>
                                                    <option value="M" {{ $eleve->sexe == 'M' ? 'selected' : '' }}>M
                                                    </option>
                                                    <option value="F" {{ $eleve->sexe == 'F' ? 'selected' : '' }}>F
                                                    </option>
                                                </select>
                                            </div>
                                                ...
                                                <select name="annee_scolaire_id" class="form-control-modern">
                                                    ...
                                                    @foreach ($annees_scolaires as $annee)
                                                        <option value="{{ $annee->id }}"
                                                            {{ $eleve->annee_scolaire_id == $annee->id ? 'selected' : '' }}>
                                                            {{ $annee->annee }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="form-group"><label class="form-label">Responsable</label><input
                                                    name="responsable" class="form-control-modern"
                                                    value="{{ $eleve->responsable }}"></div>
                                            <div class="form-group"><label class="form-label">Téléphone
                                                    responsable</label><input name="tel_responsable"
                                                    class="form-control-modern" value="{{ $eleve->tel_responsable }}">
                                            </div>
                                            <div class="form-group"><label class="form-label">Adresse</label>
                                                <textarea name="adresse" class="form-control-modern">{{ $eleve->adresse }}</textarea>
                                            </div>
                                            <div class="form-group"><label class="form-label">École de
                                                    provenance</label><input name="ecole_provenance"
                                                    class="form-control-modern" value="{{ $eleve->ecole_provenance }}">
                                            </div>
                                            <div class="form-group"><label class="form-label">Photo</label>
                                                @php $previewId = 'editElevePhotoPreview' . $eleve->id; @endphp
                                                <div style="margin-bottom:0.5rem">
                                                    <img id="{{ $previewId }}"
                                                        src="{{ $eleve->photo ? '/' . $eleve->photo : '' }}"
                                                        alt="photo"
                                                        style="max-width:120px;border-radius:6px;{{ $eleve->photo ? '' : 'display:none;' }}">
                                                </div>
                                                <input type="file" name="photo" class="form-control-modern"
    ...
    <!-- Create modal -->
    <div class="modal fade modal-modern" id="createEleveModal" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Nouvel Élève</h5><button class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <form action="{{ route('eleves.store') }}" method="POST" enctype="multipart/form-data">@csrf
                    <div class="modal-body">
                        <div class="form-group"><label class="form-label">Nom</label>
                            <input name="nom" class="form-control-modern" required>
                        </div>
                        <div class="form-group"><label class="form-label">Date de naissance</label>
                            <input type="date" name="date_n" class="form-control-modern" required>
                        </div>
                        <div class="form-group"><label class="form-label">Lieu de naissance</label><input name="lieu_n"
                                class="form-control-modern" required></div>
                        <div class="form-group"><label class="form-label">Sexe</label>
                            <select name="sexe" class="form-control-modern" required>
                                <option value="">--Choisir--</option>
                        ...
                        <div class="form-group"><label class="form-label">Année scolaire</label>
                            <select name="annee_scolaire_id" class="form-control-modern" required>
                                <option value="">--Aucune--</option>
                                @foreach ($annees_scolaires as $annee)
                        ...
                        <div class="form-group"><label class="form-label">Responsable</label><input name="responsable"
                                class="form-control-modern" required></div>
                        <div class="form-group"><label class="form-label">Téléphone responsable</label><input
                                name="tel_responsable" class="form-control-modern" required></div>
                        <div class="form-group"><label class="form-label">Adresse</label>
                            <textarea name="adresse" class="form-control-modern" required></textarea>
                        </div>
                        <div class="form-group"><label class="form-label">École de provenance</label>
                            <input name="ecole_provenance" class="form-control-modern">
                        </div>
                        <div class="form-group"><label class="form-label">Photo</label><input type="file"
                                name="photo" class="form-control-modern"
                                onchange="previewFile(event, '#createElevePhotoPreview')">
                            <div style="margin-top:0.5rem"><img id="createElevePhotoPreview" src=""
                                    alt="preview" style="max-width:120px;border-radius:6px;display:none;"></div>
                        </div>
                    </div>
                    <div class="modal-footer"><button type="button" class="btn btn-secondary"
